...
public class Main {
    ...
    // есть 4 кнопки для выбора напитка: эспрессо, американо, капучино, чай. Вам необходимо написать алгоритм,
    // используя switch-case конструкцию для создания этих напитков, процесс приготовления которых будет напечатан в
    // консоли. При этом, каждый напиток включает в себя определенные действия (элементарные процессы, будут описаны
    // ниже), которые можно упаковать в самостоятельные методы, для их многократного использования.
   ...
   // Выбор кнопки можно считать через командную строку (например в int значение).
    ...
    void getCoffee(int button) {
        switch (button){
            case 1 -> System.out.println("Приготовление экспрессо- минимально неразделимый процесс.");
